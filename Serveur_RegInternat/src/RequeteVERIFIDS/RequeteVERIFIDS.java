...
public class RequeteVERIFIDS implements Requete, Serializable
{
    ...
    private Properties propServeurFrontieresNational;
    ...
    public RequeteVERIFIDS(byte [] bytReq, byte [] bytReqIdentite, byte [] bytReqPays, byte [] bytReqPermis, byte [] bytReqcléSecr, byte [] bytReqCléHMAC, byte [] bytReqSignature)
    {
        ...
        propServeurFrontieresNational = LibJDBC.RecupPropertiesFrontieresNationale();
    }

    public RequeteVERIFIDS(int type, String PlaqueImmatriculatio, byte [] signature) {
        this.type = type;
    ...
    private Boolean DecryptCode(byte[] typeReq ,byte[] mess, byte [] permis, byte [] pays, byte[] signature, byte [] clésession, byte [] cléhmac) 
    {
        try {
            ...
            byte[] typeReq2 = null;
            if(typeReq != null) typeReq2 = decrypt.doFinal(typeReq);
            byte[] mess2 = null;
    ...
    private void traiteRequeteListingIdentiteInternational(Socket SocketClient2)
    {
        ...
        {   
            ...
            if (NumeroNational!= null && NuméroPermis != null)
            {
                ...
                reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTEVEHICULES_OK,bytNumReg,bytNumPermis,bytNom,bytPrenom,bytDateNaissance,bytDateMaxValiditeIdentite,bytDateMaxValiditePermis);
            }
            else
            {
                if (NumeroNational == null)
                {
                    reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTNUMNATIONAL_KO);                    
                }   
                else
                {
                    if (NuméroPermis == null)
                    {
                        reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTPERMIS_KO);
                    }
                    else
                        reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTIDENTITEPERMIS_KO);
                }
            }
    ...
    private void traiteRequeteListingPassagerIdentiteInternational(Socket SocketClient2)
    {
        ...
        {   
            ...
            if (NumeroNational!= null)
            {
                ...
                reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTPASSAGER_OK,bytNumReg,bytNom,bytPrenom,bytDateNaissance,bytDateMaxValiditeIdentite);
            }
            else
                reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTPASSAGER_KO);
    ...
    private void traiteRequeteListingVehiculesInternational(Socket SocketClient2)
    {
        ...
        {   
            ...
            s.update(String.valueOf(ReponseVERIFIDS.LISTEVEHICULES_OK).getBytes());
            byte [] signature2 = s.sign();
            if (dlm.getRowCount() > 0)
                reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTEVEHICULES_OK, dlm, signature2);
            else
                reponse = new ReponseVERIFIDS(ReponseVERIFIDS.LISTEVEHICULES_KO);
            oos.writeObject(reponse);
        }  
        catch (IOException e)
